...
public class AuthorRepository {
    ...
    public Author add(Author author) {
        logger.info(Color.RED + "начал работу метод: add, Author = " + author + Color.DEFAULT);
    ...
    public Author getById(long id) {
        ...
        session.close();
        logger.info(Color.BIRUZOVII + "выход из метода: getById(long id)" + Color.DEFAULT);
        return author;
    }

    public void updateByIdName(long id, String newName) {
        logger.info(Color.RED + "начал работу метод: getById(id= " + id + ", newName= " + newName + ")" + Color.DEFAULT);
        Session session = sessionFactory.openSession();
        Author author = session.get(Author.class, id);
        logger.info(Color.GREEN + "получен объект Author c (id= " + id + ") " + author + Color.DEFAULT);
        ...
        session.save(author);
        session.getTransaction().commit();
        logger.info(Color.BIRUZOVII + "выход из метода: updateByIdName (id= " + id + ", newName= " + newName + "), статус транзакции: " + session.getTransaction().getStatus().name() + " - конечный объект = " + author + Color.DEFAULT);
        session.close();
    }
