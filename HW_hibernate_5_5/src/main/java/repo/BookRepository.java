...
//                logger.info(Color.BIRUZOVII + "в методе add20BookEntityRnd произведено выполнение метода flush() на количестве объектов = " + countEntity + Color.DEFAULT);
    ...
    public List<BookAuthor> getAllBookAuthor() {
        logger.info(Color.RED + "начал работу метод: getAllBookAuthor()" + Color.DEFAULT);
        Session session = sessionFactory.openSession();
        logger.info(Color.GREEN + "получена Session" + Color.DEFAULT);
        String sql = "SELECT b.id, b.name_book, a.name, a.last_name FROM books b INNER JOIN authors a ON a.id = b.author_id";
        logger.info(Color.GREEN + "создан запрос String sql: " + sql + Color.DEFAULT);
        NativeQuery<BookAuthor> nativeQuery = session.createNativeQuery(sql);
        logger.info(Color.GREEN + "создан нативный запрос createNativeQuery(sql)" + Color.DEFAULT);
        nativeQuery.addEntity(BookAuthor.class);
        logger.info(Color.GREEN + "получены Entity (BookAuthor.class) c помощью NativeQuery(sql)" + Color.DEFAULT);
        List<BookAuthor> bookAuthorList = nativeQuery.list();
        logger.info(Color.GREEN + "получен List<BookAuthor> c помощью nativeQuery.list()" + Color.DEFAULT);
        session.close();
        logger.info(Color.BIRUZOVII + "закончил работу метод: getAllBookAuthor()" + Color.DEFAULT);
